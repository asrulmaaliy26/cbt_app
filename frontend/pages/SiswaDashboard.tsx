...
const SiswaDashboard: React.FC<SiswaDashboardProps> = ({ user, exams, setActivities, activities, externalExamUrl, returnUrl, externalExamId, externalExamTitle, externalExamType }) => {
  ...
  const saveViolation = async (reason: string, detail: string) => {
    ...
    try {
      await fetch('http://localhost:8080/api/violations/save', {
        method: 'POST',
      ...
      if (lastFrameDataRef.current) {
        ...
        if (noMotionCounterRef.current > 20) {
          ...
          setActivities(prev => {
            ...
            // If not executing normal exam flow but external, might not be in activities yet if we didn't add it in confirmStart correctly for external (fixed below)
            return prev;
          });
  ...
  useEffect(() => {
    ...
    const handleWindowBlur = () => {
      // Only trigger if document is NOT hidden (meaning it's visible but lost focus, likely overlay or another app)
      // OR if document IS hidden, we already catch it with visibilityChange. 
      // Note: visibilityChange is more reliable for tab switching. Blur captures clicking outside browser (e.g. calculator app).
      if (!document.hidden) {
        handleViolation('APP_SWITCH', 'Mahasiswa membuka aplikasi lain atau klik di luar browser.');
      }
    };

    // Helper to handle violations updates state and saves to DB/Iframe
    const handleViolation = (type: string, detail: string) => {
      setSuspiciousAlert(`PERINGATAN: ${type === 'EXIT_FULLSCREEN' ? 'Dilarang keluar Fullscreen!' : 'Dilarang beralih fokus/aplikasi!'}`);

      if (type === 'TAB_SWITCH') localTabSwitchCountRef.current += 1;
      if (type === 'EXIT_FULLSCREEN') localFullscreenExitCountRef.current += 1;
      if (type === 'APP_SWITCH') localAppSwitchCountRef.current += 1;
  ...
  const confirmStart = () => {
    if (!isCameraReady && (activeExam?.isCameraRequired || externalExamUrl)) { return alert('Kamera wajib aktif.'); }
      ...
      return (
        <div className="max-w-2xl mx-auto space-y-8 py-10">
          ...
          <button
            ...
            className={`w-full py-5 rounded-2xl font-black text-xl shadow-xl flex items-center justify-center gap-3 transition-all ${isCameraReady ? 'bg-emerald-600 text-white hover:bg-emerald-700' : 'bg-slate-100 text-slate-400 cursor-not-allowed'}`}
          >
