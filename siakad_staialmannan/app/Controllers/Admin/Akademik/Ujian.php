                    ...
                    if ($this->request->getVar('jns_ujian') == 'UTS') {
                        ...
                        if ($dtMk['status_uts'] == '0') {
                            ...
                            if (($dtMk['jns_uts'] == '2') && !empty($dtMk['uts_soal'])) {
                                $soal_uts = '<a href="javascript:void(0)" class="btn btn-success btn-xs" onclick="showSoal(' . "'uts','" . $list['id_mk'] . "','" . $list['id_ljk'] . "','" . str_replace("'", "`", $dtMk['Mata_Kuliah']) . "'" . ')">Esai / Penugasan</a>';
                            }
                            if (($dtMk['jns_uts'] == '2') && empty($dtMk['uts_soal'])) {
                                $soal_uts = '<span class="badge badge-danger">Soal Belum Diinput</span>';
                            }
                            if ($dtMk['jns_uts'] == '1') {
                                $soal_uts = '<a href="javascript:void(0)" class="btn btn-success btn-xs" onclick="showSoal(' . "'uts','" . $list['id_mk'] . "','" . $list['id_ljk'] . "','" . str_replace("'", "`", $dtMk['Mata_Kuliah']) . "'" . ')">Artikel</a>';
                            }
                        }
                    ...
                    if ($this->request->getVar('jns_ujian') == 'UAS') {
                        ...
                        if ($dtMk['status_uas'] == '0') {
                            ...
                            if (($dtMk['jns_uas'] == '2') && !empty($dtMk['uas_soal'])) {
                                $soal_uas = '<a href="javascript:void(0)" class="btn btn-success btn-xs" onclick="showSoal(' . "'uas','" . $list['id_mk'] . "','" . $list['id_ljk'] . "','" . str_replace("'", "`", $dtMk['Mata_Kuliah']) . "'" . ')">Esai / Penugasan</a>';
                            }
                            if (($dtMk['jns_uas'] == '2') && empty($dtMk['uas_soal'])) {
                                $soal_uas = '<span class="badge badge-danger">Soal Belum Diinput</span>';
                            }
                            if ($dtMk['jns_uas'] == '1') {
                                $soal_uas = '<a href="javascript:void(0)" class="btn btn-success btn-xs" onclick="showSoal(' . "'uas','" . $list['id_mk'] . "','" . $list['id_ljk'] . "','" . str_replace("'", "`", $dtMk['Mata_Kuliah']) . "'" . ')">Artikel</a>';
                            }
                        }
                        $row[] = $no;
                        $row[] = $dtMk['Mata_Kuliah'];
                        ...
                        $row[] = (!empty($dtMk['tugas'])) ? '<a href="javascript:void(0)" class="btn btn-success btn-xs" onclick="showSoal(' . "'tugas','" . $list['id_mk'] . "','" . $list['id_ljk'] . "','" . str_replace("'", "`", $dtMk['Mata_Kuliah']) . "'" . ')">Tugas Akhir</a>' : '';
    ...
    function showSoal()
    {
        ...
        if ($this->request->getMethod(true) == 'POST') {
            ...
            if (!$this->validate($aturan)) {
                ...
                if ($this->request->getVar('catatan_pelanggaran')) {
                    $db = \Config\Database::connect();
                    $existing = $db->table('data_ljk')->where('id_ljk', $this->request->getVar('id_ljk'))->get()->getRowArray();

                    if ($this->request->getVar('jns_ujian') == 'uts') {
                        $prev = $existing['ctt_uts'] ?? '';
                        $record['ctt_uts'] = $prev . "\n" . date('Y-m-d H:i:s') . ' : ' . $this->request->getVar('catatan_pelanggaran');
                    }
                    if ($this->request->getVar('jns_ujian') == 'uas') {
                        $prev = $existing['ctt_uas'] ?? '';
                        $record['ctt_uas'] = $prev . "\n" . date('Y-m-d H:i:s') . ' : ' . $this->request->getVar('catatan_pelanggaran');
                    }
                }
                // dd($this->request->getVar('catatan_pelanggaran'));
